...
async function registerAndLookup() {
    ...
    const regAdd1 = "0x1757340a42245e6Ee66E0a782B2372f3ADFC34DB"
    const regAdd2 = "0xEA5aDDD0c3E0b2FB99f1313e0417A5a70EA917c5"

    const startTime = Date.now();

    console.log("starting the first call : get the adddress table size")
    // Register addresses
    const size = await contract.methods.getAddressTableSize().call();  
    console.log(`table size: ${size} `);
    console.log("starting the second call : register an address")
    await contract.methods.registerAddresses([regAdd1]).send({ from: regAdd2 });
    console.log("address registered")
    // Lookup an address that exists
    const indexKnown = await contract.methods.lookupAddress(regAdd1).call();
